...
def answer(x):
    num_cars = len(x)
    total_rabbits = sum(x)
    rabbits_in_each_equal_car = int(math.ceil(total_rabbits/float(num_cars))) #rabbits/car rounded up
    #print rabbits_in_each_equal_car

#If the total number of rabbits is less than the number of cars, there will only be
#one rabbit/car. Else if the rabbits divide evenly into the number of cars all the cars will
#be even. Otherwise, all the cars will be equal except one small one.     
    if total_rabbits <= num_cars:
        return total_rabbits
    elif total_rabbits % rabbits_in_each_equal_car == 0:
    #    print "%s mod %s" % (total_rabbits, rabbits_in_each_equal_car)
        return num_cars
    else:
    #    print "%s mod %s" % (total_rabbits, rabbits_in_each_equal_car)
        return num_cars -1
